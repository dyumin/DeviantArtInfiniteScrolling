...
struct Thumbs
{
    ...
    init?(_ thumbs: Array<Dictionary<String, Any>>)
    {
        guard let json = thumbs.last // todo: keep all thumbs
                , let src = json[Content.cisrc] as? String
            ...
            assertionFailure("\(#function): failed to create instance with \(thumbs)")
            return nil
        }
...
struct Deviaton
{
    ...
    init?(_ json: Dictionary<String, Any>)
    {
        ...
        if let thumbs = json[Deviaton.cithumbs] as? Array<Dictionary<String, Any>> {
            self.thumbs = Thumbs(thumbs)
        }
        else {
            self.thumbs = nil
        }
